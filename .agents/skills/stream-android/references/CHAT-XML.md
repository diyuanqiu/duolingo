...
- **Never instantiate Stream ViewModels in arbitrary scopes.** Use `viewModels { factory }` (or `by viewModels()` with Hilt-provided ViewModels on an `@AndroidEntryPoint` host). Creating them in a callback or local `lazy` outside the Activity/Fragment scope leaks listeners and breaks state restoration.
